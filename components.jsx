...
// ---- Tooth SVG ----
function ToothIcon({ size = 220, golden = false, pulsing = false, clicked = 0 }) {
  ...
  return (
    <svg width={size} height={size} viewBox="0 0 200 200" style={{ filter: `drop-shadow(0 10px 24px ${shadow}40)` }}>
      ...
      <path
        ...
        fill={`url(#tooth-grad-${golden ? 'g' : 'w'})`}
        stroke={stroke}
        strokeWidth="3"
      />
      {/* shine */}
...
// ---- StatsGroup: titled card with a list of label→value rows ----
...
function TabBar({ tabs, active, onChange }) {
  return (
    <div style={{ display: 'flex', gap: 2, borderBottom: '1px solid var(--border-subtle)', marginBottom: 'var(--spacing-5)' }}>
      {tabs.map(t => {
        ...
        return (
          <button
            ...
            style={{
              background: 'none',
              border: 'none',
              borderBottom: isActive ? '2px solid var(--primary-i100)' : '2px solid transparent',
              padding: 'var(--spacing-3) var(--spacing-4)',
              color: isActive ? 'var(--primary-i100)' : 'var(--fg-2)',
          ...
          >
            ...
            {t.badge != null && (
              <span style={{
                ...
                background: isActive ? 'var(--primary-i010)' : 'var(--bg-3)',
                color: isActive ? 'var(--primary-i100)' : 'var(--fg-3)',
...
// ---- GeneratorRow ----
function GeneratorRow({ gen, owned, cost, canAfford, unlocked, revealed, onBuy, lang, totalTeeth, production }) {
  ...
  if (!unlocked) {
    return (
      <div style={{
        display: 'grid', gridTemplateColumns: '48px 1fr auto', gap: 'var(--spacing-4)',
        alignItems: 'center',
        padding: 'var(--spacing-3) var(--spacing-4)',
        background: 'var(--bg-3)',
        border: '1px solid var(--border-subtle)',
        ...
        <div>
          ...
          <div className="t-body-s" style={{ color: 'var(--fg-3)' }}>
            {t.unlockAt} {formatNum(gen.unlockAt)} {t.teeth}
          </div>
        </div>
        <div className="t-body-s" style={{ color: 'var(--fg-3)' }}>
          {formatNum(Math.max(0, gen.unlockAt - totalTeeth))} {t.teeth}
        </div>
      </div>
    );
  }
  ...
  return (
    <button
      ...
      style={{
        all: 'unset',
        display: 'grid', gridTemplateColumns: '48px 1fr auto auto', gap: 'var(--spacing-4)',
        alignItems: 'center',
        padding: 'var(--spacing-3) var(--spacing-4)',
        background: 'var(--bg-1)',
        border: `1px solid ${canAfford ? 'var(--primary-i020)' : 'var(--border-subtle)'}`,
        borderRadius: 'var(--radius-m)',
    ...
    >
      <div style={{
        ...
        display: 'flex', alignItems: 'center', justifyContent: 'center',
        color: 'var(--primary-i100)',
      }}>
        <i className={gen.icon} style={{ fontSize: 20 }}></i>
      </div>
      <div style={{ minWidth: 0 }}>
        <div className="t-heading-xs" style={{ color: 'var(--fg-1)' }}>{name}</div>
        <div className="t-body-s" style={{ color: 'var(--fg-3)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{desc}</div>
      </div>
      <div style={{ textAlign: 'right', minWidth: 90 }}>
        <div className="t-mini-caps" style={{ color: 'var(--fg-3)' }}>{t.owned}</div>
        <div className="t-heading-s" style={{ color: 'var(--fg-1)', fontVariantNumeric: 'tabular-nums' }}>{owned}</div>
        {owned > 0 && (
          <div className="t-body-s" style={{ color: 'var(--positive-i100)', fontVariantNumeric: 'tabular-nums' }}>
            +{formatNum(production)}/s
          </div>
        )}
      </div>
      <div style={{ textAlign: 'right', minWidth: 110 }}>
        ...
        <div className="t-heading-s" style={{ color: canAfford ? 'var(--primary-i100)' : 'var(--fg-2)', fontVariantNumeric: 'tabular-nums', display: 'flex', alignItems: 'center', gap: 6, justifyContent: 'flex-end' }}>
          <i className="fa-solid fa-tooth" style={{ fontSize: 13, color: 'inherit' }}></i>
          {formatNum(cost)}
        </div>
      </div>
    </button>
  ...
  if (!unlocked) {
    return (
      <div style={{
        ...
        <div>
          <div className="t-heading-xs" style={{ color: 'var(--fg-3)' }}>???</div>
          <div className="t-body-s" style={{ color: 'var(--fg-3)' }}>{t.unlockAt} {formatNum(up.unlockAt)} {t.teeth}</div>
        </div>
        <div className="t-body-s" style={{ color: 'var(--fg-3)' }}>
          {formatNum(Math.max(0, up.unlockAt - totalTeeth))} {t.teeth}
        </div>
      </div>
    );
  }
  ...
  if (purchased) {
    return (
      <div style={{
        ...
        alignItems: 'center',
        padding: 'var(--spacing-3) var(--spacing-4)',
        background: 'var(--positive-i010)',
        border: '1px solid var(--positive-i050)',
        borderRadius: 'var(--radius-m)',
      }}>
...
function AchievementCard({ ach, unlocked, lang }) {
  ...
  return (
    <div style={{
      display: 'grid', gridTemplateColumns: '48px 1fr', gap: 'var(--spacing-3)',
      alignItems: 'center',
      padding: 'var(--spacing-3) var(--spacing-4)',
      background: unlocked ? 'var(--warning-i010)' : 'var(--bg-3)',
      border: `1px solid ${unlocked ? 'var(--warning-i050)' : 'var(--border-subtle)'}`,
      borderRadius: 'var(--radius-m)',
      opacity: unlocked ? 1 : 0.75,
    }}>
      <div style={{
        width: 48, height: 48, borderRadius: 'var(--radius-s)',
        background: unlocked ? 'var(--warning-i100)' : 'var(--bg-1)',
        display: 'flex', alignItems: 'center', justifyContent: 'center',
      ...
      <div>
        <div className="t-heading-xs" style={{ color: unlocked ? 'var(--warning-i130)' : 'var(--fg-3)' }}>
          {unlocked ? name : '???'}
        </div>
...
function Toast({ toast, lang }) {
  ...
  return (
    <div style={{
      ...
      background: 'var(--complementary-i080)',
      color: '#fff',
      padding: 'var(--spacing-3) var(--spacing-4)',
      borderRadius: 'var(--radius-m)',
...
Object.assign(window, { ToothIcon, StatTile, TabBar, GeneratorRow, ClickUpgradeRow, AchievementCard, Toast });
