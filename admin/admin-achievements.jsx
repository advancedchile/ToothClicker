...
window.AdminAchievements = function({ lang, isTemplate, setToast }) {
    ...
    if (isAutoGenerated(desc_es) || isAutoGenerated(desc_en)) {
      if (formData.reqType && formData.reqValue !== undefined) {
        ...
        if (formData.reqType === 'specific_generator_count' && formData.reqTargetId) {
          ...
          if (gen) suffixEs = ` - Gen: ${gen.name?.es || gen.es || (typeof gen.name === 'string' ? gen.name : '')}`;
        }

        const val = window.formatNum ? window.formatNum(formData.reqValue) : formData.reqValue;
        const newDescEs = `Req: ${reqLabelEs} (${val})${suffixEs}`;
  ...
  const handleSaveForm = () => {
    ...
    if (!desc_es || !desc_en) {
      ...
      const val = window.formatNum ? window.formatNum(formData.reqValue) : formData.reqValue;
      const autoEs = `Req: ${reqLabelEs} (${val})${suffixEs}`;
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--fg-3)' }}>
                      Req: {reqLabel} ({window.formatNum(item.reqValue)})
                      {item.reqType === 'specific_generator_count' && ` - Gen: ${generators.find(g => g.id === item.reqTargetId)?.name?.es || generators.find(g => g.id === item.reqTargetId)?.es || '?'}`}
                    </div>
                  </div>
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
