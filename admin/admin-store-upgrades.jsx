...
window.AdminStoreUpgrades = function({ lang, setToast }) {
  ...
  const handleBenefitBlur = (overrideData = {}) => {
    setFormData(prev => {
      const currentGenId = overrideData.generatorId !== undefined ? overrideData.generatorId : prev.generatorId;
      const currentMult = overrideData.multiplier !== undefined ? overrideData.multiplier : prev.multiplier;

      let desc_es = (prev.description?.es || '').trim();
      let desc_en = (prev.description?.en || '').trim();
      ...
      let newDescEs = desc_es;
      let newDescEn = desc_en;
      ...
      if (isAutoGenerated(desc_es) || isAutoGenerated(desc_en)) {
        ...
        if (currentGenId) {
          const genNameEs = getGenName(currentGenId);
          const gen = generators.find(g => g.id === currentGenId);
          const genNameEn = gen?.name?.en || gen?.en || (typeof gen?.name === 'string' ? gen.name : '(No name)');
          if (currentMult) {
            autoDescEs.push(`- Multiplica la producción de ${genNameEs} x${currentMult}`);
            autoDescEn.push(`- Multiplies ${genNameEn} production x${currentMult}`);
          }
        } else {
          if (currentMult) {
            autoDescEs.push(`- Multiplica la producción global x${currentMult}`);
            autoDescEn.push(`- Multiplies global production x${currentMult}`);
          }
        }

        if (isAutoGenerated(desc_es) && autoDescEs.length) newDescEs = autoDescEs.join('\n');
        if (isAutoGenerated(desc_en) && autoDescEn.length) newDescEn = autoDescEn.join('\n');
      }

      return {
        ...prev,
        ...(overrideData.generatorId !== undefined ? { generatorId: overrideData.generatorId } : {}),
        ...(overrideData.multiplier !== undefined ? { multiplier: overrideData.multiplier } : {}),
        description: {
          ...prev.description,
          es: newDescEs,
          en: newDescEn,
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            ...
            <div>
              ...
              <window.Dropdown 
                ...
                onChange={val => handleBenefitBlur({ generatorId: val })} 
                options={[
            ...
            <div>
              ...
              <window.AdminNumberInput step="0.1" className="app-input" value={formData.multiplier} onChange={e => handleBenefitBlur({ multiplier: e.target.value })} style={{ width: '100%' }} />
            </div>
