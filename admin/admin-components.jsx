...
window.AdminHelpIcon = function({ title, text }) {
  ...
  return (
    <>
      ...
      ></i>
      {isOpen && (
        <window.Modal onClose={(e) => { if(e) e.stopPropagation(); setIsOpen(false); }} maxWidth={400} persistent={false}>
          <div style={{ padding: '20px 10px', textAlign: 'left' }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <button className="app-btn" onClick={(e) => { e.stopPropagation(); setIsOpen(false); }} style={{ background: 'var(--primary-i100)', color: '#fff', padding: '8px 16px' }}>
                Entendido
              </button>
            </div>
          </div>
