...
// ==========================================================================
// Scores are stored as { scores: [{ name, totalEarned, prestige, timePlayed, teeth, updatedAt }] }
// Uploads are merged by name (case-insensitive). Latest write wins per user.
...
async function cloudFetchLeaderboard() {
  try {
    ...
    // X-Bin-Meta: false returns the raw record directly
    const record = data && data.record !== undefined ? data.record : data;
    const scores = (record && record.scores) || [];
    // sort by prestige desc, then totalEarned desc
    scores.sort((a, b) => {
    ...
    return { ok: false, error: e.message || 'network', scores: [] };
  }
}

// ---- submit score (merge-by-name) ----
// We need to GET current bin, merge our entry, then PUT the whole thing.
// Guarded against overlapping writes with a simple in-flight lock.
let _writeInFlight = null;

async function cloudSubmitScore(entry) {
  // entry: { name, totalEarned, prestige, timePlayed, teeth }
  if (!entry || !entry.name) return { ok: false, error: 'no-name' };

  // If a write is already in flight, chain after it to avoid clobbering.
  const run = async () => {
    try {
      // 1) fetch current
      const getRes = await fetch(JSONBIN_LATEST, {
        method: 'GET',
        headers: { 'X-Master-Key': JSONBIN_MASTER, 'X-Bin-Meta': 'false' },
      });
      ...
      // 2) merge (case-insensitive by name)
      ...
      if (idx >= 0) scores[idx] = merged;
      else scores.push(merged);

      // cap the bin at 500 entries to stay under JSONBin size limits
      scores.sort((a, b) => {
        if ((b.prestige || 0) !== (a.prestige || 0)) return (b.prestige || 0) - (a.prestige || 0);
        return (b.totalEarned || 0) - (a.totalEarned || 0);
      });
  ...
  if (_writeInFlight) {
    _writeInFlight = _writeInFlight.then(run, run);
  } else {
    _writeInFlight = run();
  }
  const result = await _writeInFlight;
  // release if this was the latest
...
async function cloudDeleteScore(name) {
  ...
  const run = async () => {
    try {
      ...
      const putRes = await fetch(JSONBIN_BASE, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'X-Master-Key': JSONBIN_MASTER },
        body: JSON.stringify({ scores }),
      });
      if (!putRes.ok) throw new Error('PUT ' + putRes.status);
      return { ok: true };
...
Object.assign(window, { cloudFetchLeaderboard, cloudSubmitScore, cloudDeleteScore });
